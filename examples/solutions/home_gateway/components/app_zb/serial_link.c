...
teSL_Status eSL_Init(void)
{
    ...
    if(pdPASS != xTaskCreate((TaskFunction_t )serialReadTask,
                            "serialReadTask",
                            ...
                            NULL))
    {
        HAL_Printf("Create serialReadTask failed\r\n");
    }

    /* Start the callback handler task */
    if(pdPASS != xTaskCreate((TaskFunction_t )serialCallbackTask,
                            "serialCallbackTask",
                            ...
                            NULL))
    {
        HAL_Printf("Create serialCallbackTask failed\r\n");
    }
...
teSL_Status eSL_SendMessage(uint16_t u16Type, uint16_t u16Length, void *pvMessage, uint8_t *pu8SequenceNo)
{
    ...
    if (eStatus == E_SL_OK)
    {
        ...
        {           
            ...
            HAL_Printf("eSL_SendMessage() failed, reason = %d\r\n", eStatus);
        }
    }
...
//            taskEXIT_CRITICAL();
           ...
           // taskEXIT_CRITICAL();
            ...
            {
                HAL_Printf("Waiting Msg (0x%x) timed out\r\n", u16Type);
    ...
    HAL_Printf("No free queue slots in Msg Queue\r\n");
            ...
            if (u16Type == E_SL_MSG_STATUS)
            {
                ...
                if (psWaitStatus)
                {
                    ...
                    if (psWaitStatus->u16MessageType != ntohs(psRxStatus->u16MessageType))
                    {
                        psSerialLink->asReaderMessageQueue[i].u16Type = 0;
  //                      taskEXIT_CRITICAL();
                        HAL_Printf("Not the status listener for this Msg\r\n");
...
static void serialReadTask(void * pvParameters)
{
	...
	while (1) {
	        ...
	        else
		{
                ...
                {
                   HAL_Printf("Enqueueing message attempt\r\n");
                }				
		}
                ...
                && (sMessage.u16Type != E_SL_MSG_NODE_COMMAND_ID_LIST))
			{
            	...
            	for (psCurrentEntry = psSerialLink->sCallbacks.psListHead; psCurrentEntry; psCurrentEntry = psCurrentEntry->psNext)
            	{
                	if (psCurrentEntry->u16Type == sMessage.u16Type)
                	{
                    	...
                    	{
                        	HAL_Printf("Queue callback message failed\r\n");
        	...
        	if (0 == iHandled)
        	{
            	HAL_Printf( "Message 0x%04X was not handled\r\n", sMessage.u16Type);
        	}
